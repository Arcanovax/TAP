...
pub fn draw_room_discuss(world: &mut World, frame: &mut Frame, name: String, sentence: Vec<char>) {
    ...
    let inner_discuss = right_layout[1].inner(Margin {
        horizontal: 1,
        vertical: 1,
    });
    ...
    scroll_output = ScrollView::new(Size {
        width: content_width,
        height: content_height,
    });

    let discuss_content = Paragraph::new(Text::from(lines)).wrap(Wrap { trim: true });
    ...
    frame.render_stateful_widget(
        scroll_output,
        inner_discuss,
        &mut world.room.discuss_scroll_pos,
    );
