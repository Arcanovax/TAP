...
pub fn login_draw(world: &mut World, frame: &mut Frame) {
    let layout = Layout::default()
        .direction(Vertical)
        .constraints(vec![Percentage(50), Percentage(25), Percentage(25)])
        .split(frame.area());
    ...
    let username_area = layout[2].centered(Percentage(30), Length(3));
    ...
    if world.click {
        ...
        if !world.error {
            ...
            frame.render_widget(
                Paragraph::new(Text::from(world.message_error.as_str())).block(block_popup),
                popup_area,
            );
        }
    }
