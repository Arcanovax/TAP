...
pub fn draw_trade(world: &mut World, frame: &mut Frame) {
	...
	// ITEMS TO SELL
	let mut items_list: Vec<ListItem> = vec![ListItem::new(Line::from("Nothing").alignment(Alignment::Center))];
		...
		for (item, quantity) in &world.player.inventory {
			if let Some(item_obj) = world.list_items.get(item){
				items_list.push(ListItem::new(
					Line::from(format!("{} x{} ({} golds)", item_obj.name.clone(), quantity, item_obj.price))
					.alignment(Alignment::Center)));
			} else {
				items_list.push(ListItem::new(
					Line::from(format!("{} x{} (Un)"))
					.alignment(Alignment::Center)));
			}
		}
	}
