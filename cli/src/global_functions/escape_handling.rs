...
pub fn escape_handling(world: &mut World, frame: &mut Frame, step: u32, cancelled_instant: Option<Instant>) {
	...
	match cancelled_instant {
		Some( instant ) => {
			...
			let inner_popup = popup_area.inner(Margin { horizontal: 1, vertical: 1 });
			...
			content = vec![
						Line::from("You're so easily influenced!").centered()
					];
		...
		None => {
			...
			match step {
				...
				5 => {
					content = vec![
						Line::from("Before you go, would you like to make a donation to help protect orphaned lettuces?"),
			...
			frame.render_widget(Clear, popup_area);
			frame.render_widget(Paragraph::new(Text::from(content)).block(block_popup).centered().wrap(Wrap { trim: true }), popup_area);
		}
	}
}