use crate::{
    handlers::fight::{attack::execute_attack, is_it_my_turn::is_it_my_turn},
    protocol::{EventType, Message, Payload},
    state::SharedServer,
    structures::{
        attack_result::AttackResult,
        enums::{error::ErrorCode, npc_kind::NPCKind, state::State, turn_res::TurnRes},
        fight::Fight,
        room::Room,
    },
};
...
pub fn fight_request(
    ...
    if !is_he_there(&args[0], loc) {
        return Message::Response {
            ...
            payload: Payload::Json(serde_json::to_value("This target isn't here.").unwrap()),
        };
    }
        ...
        if let NPCKind::Enemy { defeated, hp, .. } = world_mut.world.npcs[&args[0]].kind {
            ...
            return Message::Response {
                ...
                payload: Payload::Json(
                    serde_json::to_value("This target isn't an enemy.").unwrap(),
                ),
            };
        }
    };
    ...
    if is_defeated {
        return Message::Response {
            ...
            payload: Payload::Json(
                serde_json::to_value("This target has already been defeated").unwrap(),
            ),
        };
    }
    ...
    match p_status {
        ...
        State::InFight { target_id: target } => {
            ...
            match turn_result {
                TurnRes::MyTurn => Message::Response {
                    error: ErrorCode::SUCCESS,
                    payload: Payload::Json(
