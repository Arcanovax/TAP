...
fn room_without_args_returns_invalid_args() {
    ...
    let res = room_request(&server, &vec![]);
...
fn room_right_id_returns_success() {
    let server = populated_server();
    let res = room_request(&server, &vec![String::from("room.city_square")]);
...
fn room_wrong_id_returns_room_not_found() {
    let server = populated_server();
    let res = room_request(&server, &vec![String::from("room.nowhere")]);
...
fn room_in_dungeon_by_id_returns_success() {
    ...
    let res = room_request(&server, &vec![dg_room(0)]);
