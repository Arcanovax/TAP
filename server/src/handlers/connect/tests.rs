...
fn connect_without_args_returns_invalid_args() {
    ...
    let (tx, _rx) = tx_rx();
    let result = connect_request(&[], &server, addr(1), &tx);
...
fn connect_valid_returns_success_and_registers_player() {
    let server = test_server();
    let (tx, _rx) = tx_rx();
    let result = connect_request(&["alice".to_string()], &server, addr(1), &tx);
...
fn connect_duplicate_name_returns_name_in_use() {
    ...
    let result = connect_request(&["alice".to_string()], &server, addr(2), &tx);
...
fn connect_same_addr_twice_returns_already_connected() {
    ...
    let result = connect_request(&["bob".to_string()], &server, addr(1), &tx);
...
fn connect_notifies_arrival_room_join() {
    ...
    let (tx, _rx) = tx_rx();
    connect_request(&["bob".to_string()], &server, addr(2), &tx);
