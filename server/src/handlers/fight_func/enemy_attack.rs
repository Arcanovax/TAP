...
use crate::{
    state::ServerInfo,
    structures::{
        enums::{enn_att_res::EnnAttRes, npc_kind::NPCKind, state::State},
        fight::Fight,
        npc::NPC,
        player::Player,
    },
...
pub fn enemy_attack(opponent_id: &str, world: &mut ServerInfo) -> EnnAttRes {
    ...
    if nb_fighters == 1 {
        target = &mut world
            .connections
            .get_mut(fight.fighters.get(0).unwrap())
            .unwrap()
            .player;
    } else {
        ...
        target = &mut world
            .connections
            .get_mut(&fight.fighters[target_index])
            .unwrap()
            .player;
    }
    if let NPCKind::Enemy { damages, .. } = opponent.kind {
        if damages < target.hp {
            ...
            EnnAttRes::Hit(format!(
                "{enn_name} deals {damages} damages to {pl}. Remains {hp} HP to {pl}",
                pl = target.name,
                enn_name = opponent.name,
                hp = target.hp
            ))
        } else {
            ...
            if nb_fighters == 1 {
                ...
                EnnAttRes::KillAndWin(format!(
                    "{enn_name} have killed {pl}. {enn_name} won the fight. Shame on you players!",
                    pl = target.name,
                    enn_name = opponent.name
                ))
            } else {
                fight.fighters.remove(target_index);
                EnnAttRes::Kill(format!(
                    "{enn_name} have killed {pl}.",
                    pl = target.name,
                    enn_name = opponent.name
                ))
            }
        ...
        world.fights.remove(opponent_id);
        EnnAttRes::Error(format!(
            "{enn_name} is not an enemy",
            enn_name = opponent.name
        ))
    }
}
