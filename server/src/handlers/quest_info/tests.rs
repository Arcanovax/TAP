...
fn quest_info_with_wrong_args_returns_invalid_args() {
    ...
    assert_eq!(
        quest_info_request(
            &server,
            &vec!["quest.fetch".to_string(), "extra".to_string()],
        ),
        err(ErrorCode::INVALID_ARGS)
    );
}
...
fn quest_info_unknown_quest_returns_no_quest_available() {
    let server = populated_server();
    assert_eq!(
        quest_info_request(&server, &vec!["quest.unknown".to_string()]),
...
fn quest_info_returns_the_quest_details() {
    let server = populated_server();
    let result = quest_info_request(&server, &vec!["quest.fetch".to_string()]);
