...
use crate::{protocol::{EventType, Message, Payload}, state::SharedServer, structures::{enums::{error::ErrorCode, item_kind::ItemKind, state::State}, player}};
...
pub fn consume(
	...
	let item_kind = {
		let item = world_mut.world.items.get(&args[0]);
		match item {
	...
	if let ItemKind::Potion { healing } = item_kind {
		...
		match player {
			...
			Err(code) => {
				return Message::Response { error: code, payload: Payload::Empty };
			}
		};

		let player = world_mut.get_player_mut(peer_addr).unwrap();
		player.hp = player.hp.saturating_add(healing).max(100);
		...
		match p_status {
			State::InFight { target_id } => {
				...
				for fighter in &fighters {
					...
					{
						let _ = con.tx.send(Message::Event(EventType::HEALING { player_name: p_name.clone(), heal: healing 
						}));
					} else {}
				};
			},
	...
	Message::Response { error: ErrorCode::SUCCESS, payload: Payload::Empty }

	} else {
		Message::Response { error: ErrorCode::UNUSABLE_ITEM, payload: Payload::Empty }
	}
