...
pub fn consume(peer_addr: SocketAddr, args: &[String], server_info: &SharedServer) -> Message {
    if args.len() != 1 {
    ...
    if let State::InFight { ref target_id } = p_status {
    ...
    if let ItemKind::Potion { healing } = item_kind {
        ...
        if let State::InFight { target_id } = p_status {
            let (fighters, turn) = {
                let fight = world_mut.fights.get_mut(&target_id).unwrap();
                fight.turn += 1;
