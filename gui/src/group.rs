...
pub fn draw_group(game: &mut Game) {
    ...
    if !game.group.in_group {
        let input_group_name: Rect = Rect::new(
            rect.x + (rect.w / 2.0 - (100.0)),
            rect.y + 30.0,
            200.0,
            40.0,
        );
        ...
        draw_text(
            "Group name:",
            input_group_name.x,
            input_group_name.y - 2.5,
            25.0,
            WHITE,
        );
        let input_hovered: bool = input_text(
            input_group_name,
            &mut game.group.typed,
        ...
        if let Some(invitation) = game.group.invitation.as_ref() {
            ...
            let space: f32 = 15.0;
            let join_btn: Rect =
                Rect::new(invit_rect.x + space, invit_rect.y + 50.0, btn_weight, 20.0);
            let deny_btn: Rect = Rect::new(
                invit_rect.x + invit_rect.w - btn_weight - space,
                invit_rect.y + 50.0,
                btn_weight,
                20.0,
            );
        ...
        let input_rect: Rect = Rect::new(
            rect.x + (rect.w / 2.0 - (120.0)),
            rect.y + rect.h - 100.0,
            150.0,
            35.0,
        );
        ...
        if let Some(info) = game.group.invite_info.as_ref() {
            if get_time() - info.time > 3.0 {
                ...
                if get_button(invite_btn, "Invite", 25, color, mouse)
                    && !game.group.typed.is_empty()
                {
                    send_group_invite(game);
                }
                draw_text(
                    state,
                    input_rect.x,
                    input_rect.y + input_rect.h + 15.0,
                    25.0,
                    WHITE,
                );
            }
