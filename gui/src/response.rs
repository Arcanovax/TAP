...
pub async fn handle_response(game: &mut Game, answer: &str, state: &str) {
    match &game.pending_action {
        ...
        PendingAction::Look => match serde_json::from_str::<LookData>(answer) {
            Ok(look_data) => {
        ...
        PendingAction::Items => match serde_json::from_str::<HashMap<String, ItemData>>(answer) {
            Ok(items_data) => {
        ...
        PendingAction::Npcs => match serde_json::from_str::<HashMap<String, NpcData>>(answer) {
            Ok(npcs_data) => {
        ...
        PendingAction::Status => match serde_json::from_str::<PlayerState>(answer) {
            Ok(state) => game.player.state = Some(state),
            Err(e) => {
        ...
        PendingAction::Attack(ref npc_id) => {
            if state == "OK" {
                            ...
                            if let Some(ref mut fight) = &mut game.active_fight {
                                ...
                                if fight_data.target_hp <= 0 {
                                    fight.chat.push(format!(
                                        "You deal {} damage and killed the enemy",
                                        fight_data.damage
                                    ));

                                    let mut texts =
                                        vec!["[Server] Well played, you won the fight and get:"
                                            .to_string()];
                ...
                if answer.contains("NOT_YOUR_TURN") {
                    let Some(ref mut fight) = game.active_fight else {
                        return;
                    };
                ...
                match val_str.trim().parse::<String>() {
                    Ok(taken) => {
                        if let Some(ref mut map_data) = game.map_data {
                            ...
                            let current_count =
                                game.player.inventory.data.get(&taken).copied().unwrap_or(0);
                            ...
                            if current_count > 0 {
                                ...
                                if new_count <= 0 {
                                    ...
                                    game.player
                                        .inventory
                                        .data
                                        .insert(dropped.clone(), new_count);
                                }
                            }
        ...
        PendingAction::Rooms => {
            if state == "OK" {
                if let Some(ref mut dungeon) = game.dungeon {
                    match serde_json::from_str::<std::collections::HashMap<String, RoomData>>(
                        answer,
                    ) {
                        Ok(room_data) => {
        ...
        PendingAction::SlotMachine => {
            if answer.contains("NOT_ENOUGH_GOLD") {
                game.gambling.slot = Slot {
                    result: "NO GOLD".to_string(),
                    color: YELLOW,
                }
            } else {
                if state == "OK" {
        ...
        PendingAction::Dices => {
            if answer.contains("NOT_ENOUGH_GOLD") {
                game.gambling.dice.result = Slot {
                    result: "NO GOLD".to_string(),
                    color: YELLOW,
                }
            } else {
                        ...
                        *gold -= 5;
                        ...
                        match parse_dices_data(answer) {
                            Some(data) => {
                                *gold -= 5;
                                game.gambling.dice.result = Slot {
                                    result: format!("WIN {} Gold", data.gold),
                                    color: GREEN,
                                };
                                *gold += data.gold;
                            }
