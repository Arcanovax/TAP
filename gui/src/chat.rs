...
pub fn update_chat(game: &mut Game) {
    if is_key_pressed(KeyCode::Enter) {
        if !game.chat.current_input.trim().is_empty() {
            game.chat
                .sended_messages
                .push(game.chat.current_input.clone());
            if game.chat.current_input.starts_with("/") {
                ...
                game.pending_action =
                    PendingAction::SendChat(CHANNELS[game.chat.channel as usize].to_string(), text);
            }
    ...
    if chat.is_active {
        for (i, line) in visible_msg_in_lines.iter().enumerate() {
            draw_text(
                line,
                20.0,
                bottom_y - 100.0 - (i as f32 * line_height),
                32.0,
                WHITE,
            );
        }
        ...
        for (i, line) in visible_msg_in_lines.iter().enumerate() {
            draw_text(
                line,
                20.0,
                bottom_y - 65.0 - (i as f32 * line_height),
                32.0,
                WHITE,
            );
        }
...
fn handle_direct_command(game: &mut Game) {
    ...
    if !ALLOWED_COMMANDS.contains(&command_name.as_str()) {
        ...
        game.pending_action =
            PendingAction::Command(CHANNELS[game.chat.channel as usize].to_string(), rq);
        return;
    }
