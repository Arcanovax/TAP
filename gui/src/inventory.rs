...
pub fn draw_inv(game: &mut Game) {
    ...
    handle_sroll_bar(
        ...
        total_h,
        max_offset,
        &mut game.player.inventory.inv_scroll,
    );
...
pub fn draw_dropped(game: &mut Game) {
    let Some(mapdata) = game.map_data.clone() else {
        return;
    };
...
pub fn handle_sroll_bar(
    ...
    if wheel_y != 0.0 && rect.contains(mouse) && max_offset > 0.0 {
        ...
        scroll.scroll_pos = (scroll.scroll_pos - delta).clamp(0.0, 1.0);
    }
