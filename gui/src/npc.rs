    ...
    fn handle_shop(game: &mut Game, npc: &Npc) {
        if let NPCKind::Merchant { inventory, .. } = &npc.kind {
            ...
            handle_sroll_bar(
                ...
                total_h,
                max_offset,
                &mut game.player.inventory.dropped_scroll,
            );
            ...
            for (i, item) in inventory.iter().enumerate() {
                ...
                let btn_buy = Rect::new(
                    shop_rect.x + shop_rect.w - 115.0,
                    line + (50.0 - item_size) / 2.0,
                    50.0,
                    50.0,
                );
                ...
                let info = game
                    ...
                    .buy_info
                    .as_ref()
                    .filter(|_| btn_buy.contains(game.mouse));
                ...
                let btn_sell = Rect::new(
                    shop_rect.x + shop_rect.w - 60.0,
                    line + (50.0 - item_size) / 2.0,
                    50.0,
                    50.0,
                );
                ...
                let info = game
                    ...
                    .sell_info
                    .as_ref()
                    .filter(|_| btn_sell.contains(game.mouse));
                let color = info.map(|info| info.color).unwrap_or(WHITE);
                let allowed = info.is_none();
...
pub async fn get_npc_texture(item_id: &str, kind: &NPCKind) -> Texture2D {
    let mut path = match item_id {
        ...
        "npc.farmer" => "gui/assets/npc/farmer.png",
        "npc.cow" => "gui/assets/npc/cow.png",
        "npc.dealer" => "gui/assets/npc/dealer.png",
        "npc.seller" => "gui/assets/npc/seller.png",
        "npc.citizen_3" => "gui/assets/npc/citizen_3.png",
        "npc.citizen_4" => "gui/assets/npc/citizen_4.png",
        "npc.citizen_5" => "gui/assets/npc/citizen_5.png",
        "npc.citizen_6" => "gui/assets/npc/citizen_6.png",
        "npc.door" => "gui/assets/npc/door.png",
        "npc.bookseller" => "gui/assets/npc/bookseller.png",
        "npc.client_1" => "gui/assets/npc/client_1.png",
        "npc.client_2" => "gui/assets/npc/client_2.png",
        "npc.bar_owner" => "gui/assets/npc/bar_owner.png",
...
pub fn find_npc_spawns(colliders: &[[i32; 25]; 15], tile_size: f32) -> Vec<Vec2> {
    ...
    for (row, line) in colliders.iter().enumerate() {
        for (col, &cell) in line.iter().enumerate() {
            if cell == 8 {
                positions.push(vec2(col as f32 * tile_size, row as f32 * tile_size));
            }
        }
    }
    return positions;
