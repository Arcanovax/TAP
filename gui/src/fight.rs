...
pub fn draw_enemy_info(game: &mut Game, npc: Npc) {
    let Some(fight) = &game.active_fight else {
        return;
    };
    ...
    draw_texture_ex(&npc.texture, frame.x + 12.5, frame.y, WHITE, cut_sheet_head);
...
pub fn handle_fight(game: &mut Game, floor: &Texture2D) {
    ...
    let sprite_size = vec2(
        game.config.sprite_width * 1.5,
        game.config.sprite_height * 1.5,
    );
    ...
    let btn_consume = Rect::new(
        start_x + (1.0 * (btn_size.x + spacing)),
        pos_y,
        btn_size.x,
        btn_size.y,
    );
    ...
    let btn_flee = Rect::new(
        start_x + (2.0 * (btn_size.x + spacing)),
        pos_y,
        btn_size.x,
        btn_size.y,
    );
    ...
    for (i, msg) in chat.iter().enumerate() {
        draw_text(
            msg,
            chat_rect.x + 10.0,
            chat_rect.y + (20 + (i * 20)) as f32 - offset,
            20.0,
            WHITE,
        );
    }
...
fn draw_all_players(game: &mut Game, players: HashMap<String, i32>) {
    ...
    let total_width = if players_len > 1 {
        (players_len - 1) as f32 * spacing
    } else {
        0.0
    };
