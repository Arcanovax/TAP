	...
	pub fn draw_group(game: &mut Game){
		...
		if !game.group.in_group {
			...
			if let Some(invitation) = game.group.invitation.as_ref(){
				let invit_rect: Rect = Rect::new(rect.x ,rect.y+ 40.0, 100.0, 20.0);
				let text: String = format!("{} invate you in {}",invitation.sender, invitation.group_name);
				draw_text(text, invit_rect.x+ 6.0, invit_rect.y + 22.5, 15.0, WHITE);
				let invit_btn: Rect = Rect::new(invit_rect.x,invit_rect.y + 25.0, 100.0, 20.0);
				let hovered = invit_btn.contains(Vec2::new(mouse.0, mouse.1));
				let bg = if hovered { Color::new(0.3, 0.3, 0.3, 0.75) }
				else { Color::new(0.10, 0.10, 0.10, 0.75) };
				draw_rectangle(invit_btn.x, invit_btn.y, invit_btn.w, invit_btn.h, bg);
				draw_text("Join", invit_btn.x+ 6.0, invit_btn.y + 22.5, 30.0, WHITE);
				if hovered && is_mouse_button_pressed(MouseButton::Left) {
